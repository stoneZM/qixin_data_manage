 ...
 * 积分记录模型
 */
class MpScoreRecord extends Model {
	...
	public function add_score($value,$remark='',$type='score',$flag='',$source='addon') {
		...
		if (!model('mp_score_record')->insert($insert)) {
			return false;
		} else {
			model('mp_fans')->where(array('mpid'=>$mpid,'openid'=>$openid))->setInc($type, $value);
			return true;
		}
	}
	...
	public function get_score($type='', $source='', $flag='', $openid='') {
		...
		$score_record = model('mp_score_record')->where($map)->sum('value');
		return $score_record;
	}
}

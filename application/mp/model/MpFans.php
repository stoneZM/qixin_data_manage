...
class MpFans extends Model {
	...
	function save_fans_info($openid = '') {
		...
		if ($fansInfo = $this->get_fans_info($openid)) {		// 如果粉丝存在
			...
			$map['openid'] = $openid;
			$map['mpid'] = $mpid;
			$this->where($map)->update($data);				// 如果粉丝信息存在，则保存粉丝信息
		} else {					// 如果粉丝不存在
			$this->insert($data);
		}
		return $data;
	...
	public function save_fans_location($data, $openid = '') {
		...
		if (model('MpFans')->where(array('mpid'=>get_mpid(),'openid'=>$openid))->update($data) === false) {
			return false;
		} else {
			return true;
		}
	...
	public function get_fans_list($mpid = '') {
		...
		$map['mpid'] = $mpid;
		$fans_list = model('MpFans')->where($map)->order('subscribe_time desc')->select();
		if (!$fans_list) {
			return false;
		}
		return $fans_list;

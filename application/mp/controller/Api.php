...
class Api extends \think\Controller {
	...
	public function index($id) {
		...
		//$w_data['echostr'] = $_GET['echostr'];
		//$w_data['signature'] = $_GET['signature'];
		//$w_data['nonce'] = $_GET['nonce'];
		
        if (!is_numeric($id)) {     // 接口验证传递的参数支持mpid和token两种
            $id = db('mp')->where(array('token'=>$id))->value('id');
        }
		$this->mpinfo = $wechatInfo = db('mp')->where(array('id'=>$id))->find();   // 获取公众号信息
		...
		if (!empty($_GET['echostr']) && !empty($_GET["signature"]) && !empty ($_GET["nonce"])) {  // 微信接入验证
			$wechatObj = new \WechatSdk\Wechat($this->options);
	        $wechatObj->valid();
	        exit();
		}
		$wechatObj = new \WechatSdk\Wechat($this->options);                // 实例化微信接口类
        $wechatObj->getRev();                                   // 微信接口初始化
        $origin_message = $wechatObj->getRevData();             // 获取用户消息
		/*$origin_message = array(
			"ToUserName"=>"gh_2f834a96c79a",
			"FromUserName"=>"oarHlw-g5TXjN6cUeYNOuiUVddvA",
			"CreateTime"=> "1490320421",
			"MsgType"=> "event",
			"Event"=> "subscribe",
			"EventKey"=> array(),
		);*/
		...
		//$w_data['origin_message'] = $origin_message;
        ...
        $this->mp_settings = model('MpSetting')->get_settings();    // 获取公众号全局设置

        if ($this->event != 'unsubscribe') {
            model('MpFans')->save_fans_info($this->openid);             // 保存粉丝信息
            model('MpMessage')->save_message($this->message);           // 保存消息
        }
		
        if (!empty($this->mp_settings['fans_bind_on']) && !$this->fans_info['is_bind']) {         // 开启了用户绑定且用户还未绑定
            //$bind_url = url('Mp/MobileBase/fans_bind'.config('HTTP_HOST'), array('openid'=>$this->openid, 'mpid'=>$this->mpid));
           // reply_text('你需要先<a href="'.$bind_url.'">绑定个人信息</a>才能继续使用其他功能');
           // exit();
        }
		//$license_data = @file_put_contents(ROOT_PATH . 'data/test.txt', json_encode($w_data));
    ...
    protected function begin_context($expire = 300, $context=array()) {
        ...
        cache('context_'.get_openid(), $this->addon, $expire);
        if (count($context) > 0) {
            cache('context_'.get_openid().'_context', $context, $expire);       // 缓存上下文消息内容
        }
    }
    ...
    protected function end_context() {
        cache('context_'.get_openid(), NULL);
        cache('context_'.get_openid().'_context', NULL); 
    }
    ...
    protected function get_context() {
        return cache('context_'.get_openid().'_context');
    }
    ...
    private function respond_addon($addon, $message) {
        if (!model('MpAddons')->is_addon_forbidden($this->addon, $this->mpid)) {
            /*$respond = controller('Addons://'.$this->addon.'/Respond');
    ...
    private function respond_keyword($keyword, $message) {
        if (cache('context_'.get_openid())) {       // 消息上下文存在
            $this->in_context = 1;
            /*$this->addon = cache('context_'.get_openid());
            $this->addon_settings = model('AddonSetting')->get_addon_settings($this->addon, $this->mpid);
            $respond = controller('Addons://'.$this->addon.'/Respond');
            if (method_exists($respond, 'wechat')) {
                $respond->wechat($this->message);
            }*/
                ...
                switch ($reply_type) {
                    ...
                    case 'image':
                        $articles[0] = array(
                            'Title' => '这是一张图片',
                            'Description' => '点击查看大图',
                            'PicUrl' => get_wechat_img($result['image'],'path'),
                            'Url' => get_wechat_img($result['image'],'path')
                        );
                        reply_news($articles);
                        break;
                    case 'news':
                        $articles[0] = array(
                            ...
                            'PicUrl' => get_wechat_img($result['picurl'],'path'),
                            'Url' => $result['url']
                        );
                        reply_news($articles);
                        break;
        ...
        if ($entry_rule) {      // 响应封面入口
            ...
            if (!model('MpAddons')->is_addon_forbidden($this->addon, $this->mpid)) {
                ...
                $entry = model('MpAddonEntry')->get_entry_info($entry_id);
                $articles[0] = array(
                    'Title' => $entry['title'],
                    'Description' => $entry['desc'],
                    'PicUrl' => get_wechat_img($entry['cover'],'path'),
                    'Url' => url('/mpaddon/'.$this->addon.'/mobile/'.$entry['act'], array('mpid'=>$this->mpid, 'openid'=>$this->openid))
                );
                reply_news($articles);
                exit();
            }
    ...
    public function respond_special($type, $message) {
        $auto_reply = model('MpAutoReply')->get_auto_reply_by_type($type);
        $reply_type = $auto_reply['reply_type'];
        switch ($reply_type) {
            case 'keyword':                 // 绑定关键词
                $keyword = $auto_reply['keyword'];
                $this->respond_keyword($keyword,$message);       // 响应关键词
                break;
            case 'addon':
    ...
    public function respond_event($type, $message) {
        switch ($type) {
            case 'subscribe':           // 用户关注事件
                if (!model('MpScoreRecord')->where(array('mpid'=>$this->mpid,'openid'=>$this->openid,'source'=>'system','flag'=>'subscribe'))->find()) {
                ...
                if ($this->message['EventKey'] && $this->message['Ticket']) {
                    $scene_qrcode = model('MpSceneQrcode')->where(array('mpid'=>$this->mpid,'ticket'=>get_rev_ticket()))->find();
                    if ($scene_qrcode) {
                        ...
                        model('MpSceneQrcodeStatistics')->insert($scan_data);
            ...
            case 'unsubscribe':     // 用户取消关注时设置is_subscribe为0
                // TODO 
                model('mp_fans')->where(array('openid'=>$this->openid))->value('is_subscribe', 0);
                break;
            ...
            case 'report_location':     // 用户上报地理位置时，保存用户最新位置信息
                ...
                model('MpFans')->save_fans_location($data);         // 保存用户当前所处位置
                break;
            case 'click':
        ...
        switch ($reply_type) {
            case 'keyword':                 // 绑定关键词
                $keyword = $auto_reply['keyword'];
                $this->respond_keyword($keyword,$this->message);       // 响应关键词
                break;
            case 'addon':
                $this->addon = $auto_reply['addon'];    // 分发到插件进行处理
                $this->respond_addon($this->addon, $this->message);

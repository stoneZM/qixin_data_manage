...
class Content extends Admin {
	...
	public function _initialize() {
		...
		if (empty($list[$model_id])) {
			return $this->error(lang('Without_this_model'));
		} else {
	...
	public function index() {
		if ($this->modelInfo['list_grid'] == '') {
			return $this->error(lang('List_definition_is_not_correct'), url('cms/model/edit', array('id' => $this->modelInfo['id'])));
		}
		...
		$this->assign($data);
		$this->setMeta($this->modelInfo['title'] . lang('list'));
	...
	public function add() {
		if (IS_POST) {
			...
			if ($result) {
				return $this->success(lang('add').lang('success'), url('cms/content/index', array('model_id' => $this->modelInfo['id'])));
			} else {
				return $this->error($this->model->getError(), url('cms/content/add', array('model_id' => $this->modelInfo['id'])));
			}
		} else {
			$info = array(
			...
			if ($this->modelInfo['template_add']) {
				...
				$template = 'common@public/edit';
			}
			$this->assign($data);
			$this->setMeta(lang('add'). $this->modelInfo['title']);
			return $this->fetch($template);
		}
	}
	...
	public function edit($id) {
		if (IS_POST) {
			$result = $this->model->change();
			if ($result !== false) {
				return $this->success(lang('edit').lang('success'), url('cms/content/index', array('model_id' => $this->modelInfo['id'])));
			} else {
				return $this->error($this->model->getError(), url('cms/content/edit', array('model_id' => $this->modelInfo['id'], 'id' => $id)));
			}
		} else {
			if (!$id) {
			...
			$this->assign($data);
			$this->setMeta(lang('edit') . $this->modelInfo['title']);
			return $this->fetch($template);
		}
	}

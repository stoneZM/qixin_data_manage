...
* 设置模型
*/
class Model extends \app\common\model\Base{
				...
				if (empty($data['id'])) {
					...
					if ($result) {
						...
						if (false === $id) {
							...
							return $id ? array('info'=>lang('create').lang('success'),'status'=>1) : array('info'=>lang('create').lang('fail'),'status'=>1);
						}
					}else{
						return false;
					}
					...
					if (false === $status) {
						...
						return array('info'=>lang('edit').lang('success'),'status'=>1);
					}
				}
			}else{
				return array('info'=>$this->getError(),'status'=>0);
	...
	public function del(){
		...
		if ($model['extend'] == 0) {
			$this->error = lang('The_basic_model_is_not_allowed_to_delete');
			return false;
		...
		if ($db->CheckTable($tablename)) {
			...
			if (!$result) {
				return false;
				$this->error = lang('Data_table_deletion_failed');
			}
		}
		...
		if ($result) {
			...
			$this->error = lang('Model_deletion_failed');
			return false;
		}
	}
